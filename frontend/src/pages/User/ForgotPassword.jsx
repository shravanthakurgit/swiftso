...
export default function ForgotPassword() {
  ...
  const response = await axios.post(`${backendUrl}/api/user/forgot-password`, payload, {
    ...
    <div className=" flex items-start mt-10 justify-center px-4 pb-6">
       ...
       {isLoading && (
        <div className=" fixed text-center text-white font-semibold inset-0 z-50 bg-black bg-opacity-80 flex justify-center items-center ">
          <div className="absolute flex justify-center items-center gap-4">Sending OTP. Please wait.. <AiOutlineLoading3Quarters className='animate-spin' /></div>
        </div>
      )}
        ...
        <div className=" fixed text-center m-auto text-white font-semibold inset-0 z-50 bg-black bg-opacity-80 flex justify-center items-center ">
          <div className="absolute  flex justify-center items-center gap-4">OTP Sent To Your Email (also check spam folder)  <FaRegCircleCheck /></div>
      ...
      <div className="w-full max-w-sm bg-white rounded-md shadow-md p-6 space-y-6 border">
        <h2 className="font-semibold text-gray-800 text-center text-md monst ">Email For OTP</h2>
