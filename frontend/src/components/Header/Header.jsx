...
import { FaAccusoft, FaArrowRight } from "react-icons/fa6";
...
const[showLogoutAlert, setShowLogoutAlert] = useState(false)
  ...
  navigate("/logout"); // this will run your Logout component logic
};
    ...
    <div className=" fixed top-0 z-50 w-[100%]">
      ...
      {showLogoutAlert && (
              <div className=" fixed text-center m-auto text-white font-semibold inset-0 z-50 bg-black bg-opacity-30 flex justify-center items-center text-xs  ">
                <div className="absolute zoom-once flex-col flex justify-center items-center gap-4 bg-white p-4 rounded py-16 px-10 top-[25%] sm:top-[33%]">
 ...
 Are you sure want to Logout  <QuestionMarkCircleIcon className='size-[20px]'/>
  ...
  <button className='p-3 w-32 rounded bg-red-200 text-red-600'  onClick={()=>setShowLogoutAlert(false)}>No</button>
      ...
      {/* Mobile menu */}
      <Dialog open={open} onClose={setOpen} className="relative z-50 lg:hidden transition-all duration-100" onClick={() => setOpen(false)}>
        <DialogBackdrop
          transition
...
<NavLink
    ...
    ` border pr-5 flex-1 items-center rounded-full w-1/2 ${
      isActive ? 'bg-[var(--primary)] font-semibold transition-all duration-500' : ''
    }`
  } 
                  >
                  <div
  ...
  className={({ isActive }) =>
    `rounded-full  ${
      isActive ? 'bg-[var(--primary)] transition-all duration-500' : 'border'
    }`
  }
>
...
<div className="ml-auto flex items-center sm:gap-5">  
 ...
 <div
      ...
      >
        ...
        {isAuthenticated ? (<button className="bg-[var(--secondary)] rounded-full flex justify-center items-center w-8 h-8 pb-[0.3px] poppins font-semibold text-[var(--primary)] mt-1 ">{userData.first_name?.charAt(0).toUpperCase()}</button>):(<MdAccountCircle className="size-[2rem] text-gray-400 hover:text-gray-500 border rounded-full" />)}
        
      </div>


  {dropdownOpen &&(
<div className="fixed text-center text-white font-semibold inset-0 z-50 bg-black bg-opacity-5 flex justify-end items-center right-0  top-5" onClick={()=>setDropdownOpen(!dropdownOpen)}>
 ...
 <NavLink key={page.name} to={page.href} className="p-2  hover:bg-gray-200 flex items-center gap-2 rounded font-semibold text-[var(secondary)} text-sm  text-[var(--secondary)]"    onClick={() => setDropdownOpen(!dropdownOpen)}>
     <page.icon className="bg-[var(--secondary)] text-white rounded-full p-1 size-[20px] border border-white"/>{page.name}
    </NavLink>
    ))}
...
{isAuthenticated ? (  <button
 ...
 <Link to="/liked">
              ...
              <div className="ml-auto flex items-center bg-[var(--primary)] px-[5px] rounded-md gap-2 p-1">
                ...
                {/* Search */}
                <div className="flex  ml-1  bg-white rounded-full mr-1 sm:hidden">
                  <a href="#" className="p-1.5 text-[var(--secondary)] hover:text-gray-500">
